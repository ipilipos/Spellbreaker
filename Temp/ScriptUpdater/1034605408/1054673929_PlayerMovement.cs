    ...
    void HandleAnimations()
    {
        ...
        if (!isGrounded)
        {
            ...
            if (rb.linearVelocity.y > 0.1f)
                characterAnimator.ChangeAnimation("Jump1");
            else if (rb.linearVelocity.y < -0.1f)
                characterAnimator.ChangeAnimation("Jump3");
            else
                characterAnimator.ChangeAnimation("Jump2");
        }
        else if (isMoving)
        {
            // Moving on ground
            if (isRunning && Mathf.Abs(rb.linearVelocity.x) > walkSpeed * 0.8f)
                characterAnimator.ChangeAnimation("Run");
            else
                characterAnimator.ChangeAnimation("Walk");
        }
        else
        {
            // Idle on ground
